...
\begin{document}
	...
	\section{Laurent Series, Zeros and Singularities} % (fold)
	\label{sec:laurent_series_zeros_and_singularities}
		...
		\begin{proof}[Proof Sketch]
			\begin{multicols}{2}
				We start by proving existence. Fix 
				$z\in\{\rho_1\le|z-z_0|\le \rho_2\} = A_{\rho_1,\rho_2}$. Let $C_1,\:C_2$ be two positively oriented circles of radii $\tilde{\rho_1},\:\tilde{\rho_2}$ (dashed in figure), where $\tilde{\rho_1}< \rho_1<\rho_2<\tilde{\rho_2}$.

				By Cauchy's Integral Formula, we have
				\begin{align}
				\label{eq:laurent_proof}
					f(z) &=
					\frac{1}{2\pi i}\int\limits_{C_2}\frac{f(\zeta)}{(\zeta-z)}d \zeta \\ &-
					\frac{1}{2\pi i}\int\limits_{C_1}\frac{f(\zeta)}{(\zeta-z)}d \zeta\nonumber
				\end{align}
				\begin{figure}[H]
				\centering
					\begin{tikzpicture}[scale = 1.3, transform shape]
						\begin{axis}
							[
								...
								\addplot[thick, dashed, samples=100, domain=0:2*pi]
									({0.2*cos(deg(x))},{0.2*sin(deg(x))});

								\addplot[thick, samples=100, domain=0:2*pi]
									({0.5*cos(deg(x))},{0.5*sin(deg(x))}) node at (axis cs:0.4,0.4) {$\rho_1$};

								\addplot[thick, dotted, samples=100, domain=0:2*pi]
									({0.8*cos(deg(x))},{0.8*sin(deg(x))});

								\addplot[thick, samples=100, domain=0:2*pi]
									({1.2*cos(deg(x))},{1.2*sin(deg(x))}) node at (axis cs:0.9,0.9) {$\rho_2$};
		...
		\begin{example}
			...
			\end{align*}
		\end{example}
		\subsection{Zeros and Singularities} % (fold)
		\label{sub:zeros_and_singularities}
			...
			\begin{proof}[Proof Sketch]
				...
				\end{align*}
			\end{proof}
		% subsection zeros_and_singularities (end)
	% section laurent_series_zeros_and_singularities (end)
	\section{Zeros and Singularities} % (fold)
	\label{sec:zeros_and_singularities}
		...
		\label{sub:the_residue_theorem}
			...
			\label{thm:residue_thm}
				...
				\end{align*}
			\end{thm}
		% subsection the_residue_theorem (end)
	% section zeros_and_singularities (end)
	\section{Integral Calculations Using Residue Calculus} % (fold)
	\label{sec:integral_calculations_using_residue_calculus}
		...
		\subsection{The Residue Calculus} % (fold)
		\label{sub:the_residue_calculus}
			...
			\begin{thm}
				...
				\end{align*}
			\end{thm}
		% subsection the_residue_calculus (end)
		\subsection{Calculations of Integrals} % (fold)
		\label{sub:calculations_of_integrals}
					...
					\centering
						\begin{tikzpicture}
							\begin{axis}
								...
								\addplot[red, thick, samples=100, domain=0:pi]({cos(deg(x))},{sin(deg(x))}) node at (axis cs:0.5,0.7) {$C_R^+$};
								\addplot[blue, thick, samples = 100, domain = -1:1]{0} node at (axis cs:-0.5,0.2) {$[-R,R]$};
							\end{axis}
						\end{tikzpicture}
					\end{figure}
				\end{multicols}
			...
			\end{rem}
		% subsection calculations_of_integrals (end)
	% section integral_calculations_using_residue_calculus (end)
	\section{More About Integral Calculations} % (fold)
	\label{sec:more_about_integral_calculations}
		...
		\begin{example}
			...
			\end{align*}
		\end{example}
		\subsection{Principal Values} % (fold)
		\label{sub:principal_values}
					...
					\centering
						\begin{tikzpicture}
							\begin{axis}
								...
								\addplot[red, thick, samples=100, domain=0:pi]
									({cos(deg(x))},{sin(deg(x))}) node at (axis cs:0.5,0.7) {$C_R$};

								\addplot[red, thick, samples=100, domain=0:pi]
									({0.1*cos(deg(x))},{0.1*sin(deg(x))}) node at (axis cs:0.1,0.2) {$C_\varepsilon$};

								\draw node at(axis cs:0.8,0.8) {$\Gamma_{R,\varepsilon}$};
								\addplot[red, thick, domain=-1:-0.1]{0};
								\addplot[red, thick, domain=1:0.1]{0};
							\end{axis}
						\end{tikzpicture}
					\end{figure}
			...
			\begin{rem}
				...
				\end{align*}
			\end{rem}
		% subsection principal_values (end)
	% section more_about_integral_calculations (end)
	\section{Some More Examples of Integral Calculations} % (fold)
	\label{sec:some_more_examples_of_integral_calculations}
		\subsection{Integrals With Branch Points} % (fold)
		\label{sub:integrals_with_branch_points}
					...
					\centering
						\begin{tikzpicture}
							\begin{axis}
								...
								\addplot[red, thick, samples=100, domain=0.02:2*pi-0.02]
									({cos(deg(x))},{sin(deg(x))}) node at (axis cs:0.5,0.7) {$C_R$};

								\addplot[red, thick, samples=100, domain=0.08:2*pi-0.08]
									({0.1*cos(deg(x))},{0.1*sin(deg(x))}) node at (axis cs:0.1,0.2) {$C_\varepsilon$};

								\draw node at(axis cs:0.8,0.8) {$\Gamma_{R,\varepsilon}$};
								\addplot[red, thick, domain=0.1:1]{0.02};
								\addplot[red, thick, domain=0.1:1]{-0.02};
							\end{axis}
						\end{tikzpicture}
					\end{figure}
			...
			\begin{example}
					...
					\centering
						\begin{tikzpicture}
							\begin{axis}
								...
								\addplot[red, thick, samples=100, domain=0:2*pi/3]
									({cos(deg(x))},{sin(deg(x))}) node at (axis cs:0.5,0.7) {$\Gamma_2$};

								\addplot[red, thick, dashed, samples=100, domain=0:2*pi/3]
									({0.1*cos(deg(x))},{0.1*sin(deg(x))}) node at (axis cs:0.1,0.2) {$\alpha$};

								% \draw node at(axis cs:0.8,0.8) {$\Gamma_{R,\varepsilon}$};
								\addplot[red, thick, domain=0:1]{0} node at (axis cs:0.5,-0.2) {$\Gamma_1$};
								% \addplot[red, thick, domain=0.1:1]{-0.02};
								\draw[red, thick] (axis cs:0,0) -- (axis cs:-0.5,0.866) node at (axis cs:-0.43,0.3) {$\Gamma_3$};
				...
				\end{align*}
			\end{example}
		% subsection integrals_with_branch_points (end)
	% section some_more_examples_of_integral_calculations (end)
	\section{The Argument Principle} % (fold)
	\label{sec:the_argument_principle}
		...
		\begin{rem}
			Note that
		\end{rem}
	% section the_argument_principle (end)
\end{document}